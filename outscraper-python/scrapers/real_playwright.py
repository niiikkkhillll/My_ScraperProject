...
const phoneEl = document.querySelector('button[data-item-id^="phone:"]') ||
                    ...
                    if cleaned_website and cleaned_website != "No website listed":
                        try:
                            # Open the website in a new tab to bypass bot protections
                            ...
                            await site_page.goto(cleaned_website, wait_until="domcontentloaded", timeout=20000)
                            ...
                            if not chosen or (chosen and any(chosen.lower().startswith(p) for p in generic_prefixes)):
                                try:
                                    ...
                                    for c in contact_links:
                                        ...
                                        seen_hrefs.add(href)
                                        try:
                                            await site_page.goto(href, wait_until='domcontentloaded', timeout=10000)
                                        except Exception:
                                            pass
                            ...
                            # Close the site page now that we've finished lookups
                            try:
